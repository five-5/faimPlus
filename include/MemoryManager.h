...
{
	STACK,    // 存储 updates or helper data structures
	HEAP
};
...
public:
	...
	memory_t* d_pageindex; // @SH the start of adjacency page index manager
	...
	uint64_t start_page_index; // @SH Holds index offset needed to locate the first page index in memory
	...
	vertex_t pageindexes_per_page; // @SH  How many page indexes can fit within a page 
	...
	uint32_t next_free_pageindex_page;  // @SH new add : Indicates the index of the next free block
	...
	*/
	...
	//! Sets number of page indexes per block
	/*!
	This function can be used to set the size of page index blocks (how many page indexes per block), computes
	the given page_size
	*/
	void setPageIndexesPerBlock();

	//! Sets the size of the queue and sets the initial pointer
	void setQueueSizeAndPosition(int size);

	// @SH
	// Sets the pageindex pointer
	void setPageIndexPosition(int size);
	...
	size_t numberEdgesInMemory(vertex_t* d_neighbours_count, bool return_count = false);
	size_t numberEdgesInMemory(vertex_t* d_neighbours_count, vertex_t vertex_offset, vertex_t number_vertices, bool return_count = false);

	//! Reports back an array with the number of pages in memory
	void numberPagesInMemory(vertex_t* d_page_count);

	//! Reports back the number of pages in memory and the accumulated pages
	size_t numberPagesInMemory(vertex_t* d_page_count, vertex_t* d_accumulated_page_count);

	//! Workbalance according to page layout
	// TODO：workBalanceCalculation主要做了啥？
...
public:
			...
			(4 * graph_parser->getNumberOfVertices())) }    
	{
		if (externalAllocation)
		{
			...
			HANDLE_ERROR(cudaMalloc(&allocation, allocation_size)); //TODO: CSRData cudaMalloc分配的内存在GPU哪里？memory manager管吗？
			...
			d_pageindex_requirements = d_block_requirements + graph_parser->getNumberOfVertices() + 1;
		...
		scoped_mem_access_counter{ memory_manager.get(), sizeof(vertex_t) * (number_rows + 1 + (4 * number_rows)) }
	{
		...
		d_pageindex_requirements = temp_memory_dispenser.getTemporaryMemory<vertex_t>(number_rows);
	...
	vertex_t* d_pageindex_requirements;        // @SH add
